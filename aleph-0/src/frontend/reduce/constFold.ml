...
let rec g env = function
  ...
  | And(x, y) when memi x env && memi y env -> Int(findi x env * findi y env)
  | Or(x, y) when memi x env && memi y env -> Int(if (findi x env + findi y env) > 0 then 1 else 0)
  ...
  | While(e1, x, e3, e4) -> While(g env e1, x, g env e3, g env e4)
  | Let((x, t), e1, e2) ->
      let e1' = g env e1 in
